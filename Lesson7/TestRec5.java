...
public class TestRec5 {

	static ArrayList<OneRec2> kopo09_AOR = new ArrayList<OneRec2>(); // OneRec2형식으로 어레이리스트 생성
	...
	public static void HeaderPrint(int kopo09_num) { // 헤더 인쇄
		...
		System.out.printf("%s%40s\n", "PAGE : " + (kopo09_num + 1),
				"출력일자 : " + kopo09_sdf.format(kopo09_current.getTime())); // 현재페이지, 출력일자
	...
	public static void statistic(int kopo09_first, int kopo09_end) { // first부터 end까지의 합계, 평균 출력
		...
		System.out.printf("합계 %15d %6d %6d% 9d %8d\n", kopo09_korSum, kopo09_engSum, kopo09_matSum, kopo09_allsum,
				kopo09_allave); // 합계 정보들 출력
		System.out.printf("평균 %15d %6d %6d% 9d %8d\n", kopo09_korAve, kopo09_engAve, kopo09_matAve,
				kopo09_allsum / (kopo09_end - kopo09_first), kopo09_allave / (kopo09_end - kopo09_first)); // 평균 정보들 출력
	...
	public static void TailPrint(int kopo09_num) { // 처음부터 end까지의 합계, 평균 출력
		...
		System.out.printf("합계 %15d %6d %6d% 9d %8d\n", kopo09_sumkor, kopo09_sumeng, kopo09_summat, kopo09_sumsum,
				kopo09_sumsum / 3); // 합계 정보들 출력
		System.out.printf("평균 %15d %6d %6d% 9d %8d\n", kopo09_sumkor / kopo09_num, kopo09_sumeng / kopo09_num,
				kopo09_summat / kopo09_num, kopo09_sumsum / kopo09_num, kopo09_sumsum / 3 / kopo09_num); // 평균 정보들 출력
	}

	// 합계 순서대로 정렬
	public static void dataSort() {
		Comparator<OneRec2> kopo09_bikyeokijun = new Comparator<OneRec2>() { // Comparator클래스를 생성
			public int compare(OneRec2 kopo09_a1, OneRec2 kopo09_a2) { // Comparator클래스에 해당 메소드 추가
				return (kopo09_a2.sum() - kopo09_a1.sum()); // 합계의 차를 리턴해 kopo09_bikyeokijun에 저장
			}
		};
		Collections.sort(kopo09_AOR, kopo09_bikyeokijun); // 정렬
	}
	...
	public static void main(String[] args) {
		...
		for (; kopo09_i < kopo09_AOR.size(); kopo09_i++) { // 모든 사람에 대해
			...
			if ((kopo09_i + 1) % kopo09_division == 0) {
				statistic(kopo09_i - kopo09_division+1, kopo09_i + 1); // 현재페이지의 평균,총점
				TailPrint(kopo09_i + 1); // 누적 평균,총점
			}
		}
		if (kopo09_i % kopo09_division != 0) {	// 남은 학생에 대해
			statistic(kopo09_i / kopo09_division * kopo09_division, kopo09_i); // 총점과 평균 출력
