	...
	// 이마트 영수증, 글자크기 10
		public static void receipt3() {
			...
			String[] kopo09_itemname = { "초코파이", "바나나우유", "건포도", "오렌지주스", "초코에몽", "마스크", "식용유1.8L", "컴포트그립가위", "무말랭이 150g",
					...
					"오뚜기 진비빔면 156g", "(달콤)순살닭강정(g)", "CJ 비비고칩 20g", "1+ 동급란 10개입 특", "물티슈 캡형", "포르쉐 718" }; // 구매 상품명
			...
			for (; kopo09_count < kopo09_itemname.length; kopo09_count++) { // 모든 구매 항목에 대해
				String kopo09_newText = ""; // 상품명 변경용
				...
				System.out.printf("%s%11s%3d%11s\n", calByte(kopo09_newText), kopo09_df.format(kopo09_price[kopo09_count]),
						kopo09_num[kopo09_count], kopo09_df.format(kopo09_amount)); // calByte함수를 사용.
			...
			System.out.printf("%s%18s%9s\n", "금회발생포인트", "9350**9995", kopo09_df.format(kopo09_totalAmount/1000));
			System.out.printf("%s%15s(%8s)\n", "누계(가용)포인트", kopo09_df.format(kopo09_point+kopo09_totalAmount/1000), kopo09_df.format(kopo09_point));
			System.out.printf("*신세계포인트 유효기간은 2년입니다.\n");
			System.out.printf("-----------------------------------------\n");
			System.out.printf("%23s\n", "구매금액기준 무료주차시간 자동부여"); // 소비자 차량 정보
			System.out.printf("%s%29s\n", "차량번호 : ", "34저****");
			System.out.printf("%s%30s\n", "입차시간 : ", kopo09_sdf2.format(kopo09_current.getTime()));
			System.out.printf("-----------------------------------------\n");
